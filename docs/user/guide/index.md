...
Start the Web UI through the [root README](../../../README.md#run) (`dsh web` in a browser, or `dsh desktop` / the packaged desktop app in a window); the command prints its URL when it starts a new host. This guide begins after that server is running. The `dsh` process uses its invoking directory as the default filesystem location, but a fresh Web UI has no selected workspace until you add one.
